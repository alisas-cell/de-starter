...
4. Write `decisions.json` in `$RUN`. Stop until brand mode/profile, P1 plans, P2 choices, deletions, scoped actions, and every semantic edit's named path and purpose are approved. For a semantic edit, include its audited SHA-256, inclusive line range, replacement, and reason as specified in [input-file schemas](references/input-files.md). Omit P0; retain P1 without explicit migration and rollback.
5. Run `preview`. Show `audit.md`, `preview.md`, the private full `preview.diff`, `binary-changes.json`, `placeholders.json`, `semantic-edits.json`, protected/retained items, validation commands, unresolved work, and the exact token. Stop: approval gate two. Public updates may use redacted screenshots or a summary; never publish the full diff, private source excerpts, or private preview artifacts.
