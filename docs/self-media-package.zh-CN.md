...
录制时优先使用[中文视频导演稿](video-director-script.zh-CN.md)。导演稿已经把本包中的素材整理为时间码、屏幕画面、鼠标重点、逐字口播、证据意义和禁用表述。
...
>
> 空目录安全加固阶段的通用运行时是 195/195，CLI 13/13；独立总审还发现了普通 child move 的事务漏洞和未绑定的 preview 摘要。四条回归先真实失败，再修复，最后复审 Critical、Important、Minor 全部为 0。加入公开合成实验和拒绝边界后，当前 v0.1.2 完整回归已经扩展为 214/214。
...
### 建议补录 2–3 分钟：公开合成演示与安全边界

这一段使用 `examples/public-demo`，不要展示购买的 Starter。它是任何观众都能从 GitHub 复现的公开合成演示。

口播：

> 到这里你可能还是会担心：这个 Skill 会不会把别人的 Starter 改坏？我必须诚实地说，任何能写文件的工具都不能承诺零风险。我们能做的是把风险一层层压低，并把危险决定暴露在真正修改之前。低风险不等于零风险，Git 或可靠备份、两道人工检查和修改后的验证仍然不能省。
>
> 为了让大家不用只相信我的真实项目汇总，我又做了一个完全虚构的公开合成演示。里面故意放了 P0 版权、P1 账单 key、P2 Demo、P3 品牌文案、带来源名称的资产，以及一个需要单独批准的空目录。所有内容都叫 Northstar，和我购买的 Starter 没有关系。
>
> 第一遍我故意提交错误令牌，结果是拒绝，项目前后 inventory 完全一致，backup 和 apply-result 都没有产生。第二遍我在 Preview 之后故意改动合成文件，再提交刚才正确的旧令牌，结果仍然拒绝；Demo、资产和空目录都没有发生任何部分清理。
>
...
1. GitHub 中的 `examples/public-demo/README.md`；
2. 合成审计的 25 个文件命中和 1 个目录命中；
3. 固定 decisions 样例，P0/P1 不在修改范围；
4. 错误令牌输出，画面标注“inventory unchanged”；
5. 过期预览输出，画面标注“no partial approved edits”；
6. 当前令牌使用 `REDACTED` 遮罩；
7. `check` 输出与外部恢复材料；
8. `08-public-demo-safety.png` 总结图。

这一段的价值不是制造“绝对安全”的印象，而是让观众自己复现三个结论：错误批准凭证不会写、旧预览不会写、明确批准的当前范围才会写。
...
>
> 真正发布后，Linux CI 还发现了一个 macOS 没暴露的 inode 复用测试假设。修复后 Python 3.9、3.11、3.13 和 v0.1.1 标签 CI 全绿，生产 Skill 逻辑没有改动；后续公开演示和拒绝边界加入后，当前 v0.1.2 本地完整回归为 214/214。
>
...
7. 20 秒：公开 Linux CI 暴露测试假设；v0.1.1 完成跨平台修复，当前 v0.1.2 本地完整回归 214/214。
...
25. 提供不含购买源码的公开合成演示，观众可从 GitHub 自己复现；
26. 用项目前后 inventory 证明错误令牌没有产生任何项目写入；
27. 用 Preview 后状态变化证明旧令牌失效，并检查没有部分执行获批清理；
28. 公开区分“事务失败回滚、恢复证据”和尚不存在的“一键恢复命令”；
29. README 明确写出低风险不等于零风险，以及 Git/备份、双门检查和验证责任。
...
15. 打开 `examples/public-demo/README.md`，完整录制公开合成演示的错误令牌、过期预览和成功 Apply；
16. 插入 `08-public-demo-safety.png`，强调“低风险不等于零风险”和“没有一键恢复命令”。
...
| 公开发布复核 | `07-github-ci-green.png` | 展示历史 Linux 首次失败、三档 CI 修复与当前 v0.1.2 本地 214/214 |
| 公开安全复现 | `08-public-demo-safety.png` | 展示错误令牌、过期预览均零部分修改，以及精确批准后的保护/恢复证据 |
| 提示词与 Skill | `09-prompt-vs-skill.png` | 说明一句提示词可以启动任务，而 Skill 固化可复用的安全边界 |
| 事务与恢复 | `10-transaction-recovery.png` | 展示项目外预览、精确批准、外部备份、有限执行和失败回滚 |
| GitHub 分享卡 | `social-preview.zh-CN.png` | 用中文概括仓库用途、核心机制和“低风险不等于零风险” |
...
- [x] 替换全部 GitHub 账号占位符；
- [x] GitHub 仓库公开可访问；
- [x] README 安装命令在一个干净目录验证；
- [x] Release tag 与 CHANGELOG 一致；
- [x] 所有截图 1600×900，未裁掉状态说明；
- [x] 视频中真实 token 全程遮挡；
- [x] 不展示购买源码、私有路径、项目身份、邮箱、密钥或备份映射；
- [x] “市场比较”使用检索范围限定，不说“全网第一”；
- [x] 简介、置顶评论和片尾仓库地址一致；
- [x] 已创建 Issue 模板，邀请误报与跨技术栈反馈。
